...
impl HoughLine {
	pub fn is_horizontal(&self) -> bool {
		...
		!(PI / 60.0..=59.0 * PI / 60.0).contains(&angle)
	}

	pub fn is_vertical(&self) -> bool {
		let angle = (self.theta - PI / 2.0).abs();
		angle < PI / 60.0
	}

	pub fn intersect(&self, other: &HoughLine) -> Option<(f32, f32)> {
		let cos1 = self.theta.cos();
		let sin1 = self.theta.sin();
		let cos2 = other.theta.cos();
		let sin2 = other.theta.sin();
...
impl LineDetector {
	...
	pub fn detect_lines(&self, edges: &[bool], threshold: usize) -> Vec<HoughLine> {
		let num_thetas = (PI / self.theta_resolution) as usize;
		...
		let sin_table: Vec<f32> = (0..num_thetas)
			.map(|i| (i as f32 * self.theta_resolution).sin())
			.collect();
		let cos_table: Vec<f32> = (0..num_thetas)
			.map(|i| (i as f32 * self.theta_resolution).cos())
			.collect();
		...
		for y in 0..self.height {
			for x in 0..self.width {
				...
				for theta_idx in 0..num_thetas {
					let rho = xf * cos_table[theta_idx] + yf * sin_table[theta_idx];
		...
		for theta_idx in 0..num_thetas {
			for rho_idx in 0..num_rhos {
				...
				if votes >= threshold {
					let is_peak = self.is_local_maximum(
						&accumulator,
						theta_idx,
						rho_idx,
						num_thetas,
	...
	pub fn cluster_lines(&self, lines: &[HoughLine], distance_threshold: f32) -> Vec<HoughLine> {
		...
		for line in lines {
			...
			for cluster in &mut clusters {
				...
				if rho_diff < distance_threshold && theta_diff < PI / 90.0 {
					cluster.push(*line);
					found_cluster = true;
					break;
				}
		...
		clusters
			.into_iter()
			.map(|cluster| cluster.into_iter().max_by_key(|l| l.votes).unwrap())
			.collect()
	}
}
